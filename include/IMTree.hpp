...
Search tree (Ts) consist of an custom CSS Tree which we do not have. Therefore,
we use a STX B+tree as well.
*/
...
void IMTree<Type_Key, Type_Ts>::lookup(pair<Type_Key, Type_Ts> &arrivalTuple, Type_Key &resultCount) {
  ...
  Type_Ts lowerLimit = ((double)newTimeStamp - TIME_WINDOW < numeric_limits<Type_Ts>::min())
                           ? numeric_limits<Type_Ts>::min()
                           : newTimeStamp - TIME_WINDOW;
...
template <class Type_Key, class Type_Ts>
void IMTree<Type_Key, Type_Ts>::range_search(tuple<Type_Key, Type_Ts, Type_Key> &arrivalTuple,
                                             vector<pair<Type_Key, Type_Ts>> &searchResult) {
...
inline uint64_t IMTree<Type_Key, Type_Ts>::get_no_keys(Type_Ts Timestamp) {
  Type_Ts lowerLimit = ((double)Timestamp - TIME_WINDOW < numeric_limits<Type_Ts>::min())
                           ? numeric_limits<Type_Ts>::min()
...
inline void IMTree<Type_Key, Type_Ts>::print_stats() {
  ...
  cout << "size: "
       << sizeof(m_searchTree) + sizeof(node) * (btreeStats.leaves + btreeStats.innernodes) +
              sizeof(Type_Key) * btreeStats.innerslots * btreeStats.innernodes +
              sizeof(node *) * (btreeStats.innerslots + 1) * btreeStats.innernodes +
              sizeof(Type_Key) * btreeStats.leafslots * btreeStats.leaves +
              sizeof(Type_Ts) * btreeStats.leafslots * btreeStats.leaves + sizeof(node *) * 2
       << endl;
  ...
  cout << "avgfill_leaves: " << btreeStats.avgfill_leaves() << endl;
  cout << "size: "
       << sizeof(m_insertionTree) + sizeof(node) * (btreeStats.leaves + btreeStats.innernodes) +
              sizeof(Type_Key) * btreeStats.innerslots * btreeStats.innernodes +
              sizeof(node *) * (btreeStats.innerslots + 1) * btreeStats.innernodes +
              sizeof(Type_Key) * btreeStats.leafslots * btreeStats.leaves +
              sizeof(Type_Ts) * btreeStats.leafslots * btreeStats.leaves + sizeof(node *) * 2
