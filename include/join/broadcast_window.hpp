...
class BroadcastWindow : public SubWindow<KeyType, ValueType, Container> {
 ...
 private:
  auto ProcessR(TupleType<KeyType, ValueType> tuple) -> size_t {
    ...
    std::pair<KeyType, KeyType> key_range(tuple.key_ - diff_, tuple.key_ + diff_);
  ...
  void WorkRoutine() {
    ...
    for (const TupleType<KeyType, ValueType> tuple : *this->input_chan_) {
      if (tuple.ctl_ == TupleFlag::INPUT_R) {
        join_count += ProcessR(std::move(tuple));
      } else if (tuple.ctl_ == TupleFlag::INPUT_S) {
        join_count += ProcessS(std::move(tuple));
      } else {
  ...
  KeyType diff_;  // the difference between the two streams
};
